...
public class UserDBHelper_schedule extends SQLiteOpenHelper {

    private static final String DB_NAME="user.db";
    private static final int DB_VERSION=1;
    private static UserDBHelper_schedule mHelper=null;
    private SQLiteDatabase mDB=null;
    public static final String TABLE_NAME="user_info";

    private UserDBHelper_schedule(Context context){
        super(context,DB_NAME,null,DB_VERSION);

    }
    private UserDBHelper_schedule(Context context, int version){
        super(context,DB_NAME,null,version);
    }
    public static UserDBHelper_schedule getInstance(Context context, int version){
        if(version>0&&mHelper==null) {
            mHelper = new UserDBHelper_schedule(context, version);
        }
        else if(mHelper==null){
            mHelper=new UserDBHelper_schedule(context);
        }
        return mHelper;
    }
