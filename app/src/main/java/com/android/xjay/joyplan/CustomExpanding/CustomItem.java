...
public class CustomItem extends RelativeLayout {
    ...
    /**
     * Member variable to hold the Item Layout. Set by item_layout
     * in ExpandingItem layout.
     */
     ...
     * Can be set by {@link #setIndicatorIconRes(int)}} or
     * by {@link #setIndicatorIcon(Drawable)}.
     */
    private ImageView mIndicatorImage;

    /**
     * Member variable to hold the expandable part of indicator.
    ...
    /**
     * Member variable to hold the indicator size.
     * Set by indicator_height in ExpandingItem layout.
     */
    private int mIndicatorHeight;

    /**
     * Member variable to hold the indicator size.
     * Set by indicator_width in ExpandingItem layout.
     */
    ...
    /**
     * Member variable to hold the indicator margin at right.
     * Set by indicator_margin_right in ExpandingItem layout.
     */
    private int mIndicatorMarginRight;

    /**
     * Member variable to hold the indicator margin at top.
     * Set by indicator_margin_left in ExpandingItem layout.
     */
    private int mIndicatorMarginTop;

    /**
     * Member variable to hold the indicator margin at bottom.
     * Set by indicator_margin_left in ExpandingItem layout.
     */
    private int mIndicatorMarginBottom;

    /**
     * Member variable to hold the boolean value that defines if the
    ...
    /**
     * Member variable to hold the boolean value that defines if the
     * custom_sub_item list will start collapsed or not.
     * Set by start_collapsed in ExpandingItem layout. Default is true.
     */
    private boolean mStartCollapsed;

    /**
     * Member variable to hold the state of custom_sub_item items.
     * true if shown. false otherwise.
     */
    private boolean mSubItemsShown;

    /**
     * Member variable to hold the layout resource of custom_sub_item items.
     * Set by sub_item_layout in ExpandingItem layout.
     */
    private int mSubItemLayoutId;

    /**
     * Member variable to hold the layout resource of items.
     * Set by item_layout in ExpandingItem layout.
     */
    private int mItemLayoutId;

    /**
     * Member variable to hold the layout resource of separator.
     * Set by separator_layout in ExpandingItem layout.
     */
    ...
    public View getSubItemView(int position) {
        ...
        throw new RuntimeException("There is no custom_sub_item "
                + "custom_item for position " + position
                + ". There are only " + mBaseSubListLayout.getChildCount()
                + " in the list.");
    }
    ...
    /**
     * Remove the given view representing the custom_sub_item custom_item.
     * Should be an existing custom_sub_item custom_item.
     *
    ...
    public void removeSubItemFromList(View view) {
        if (view != null) {
            ...
            expandSubItemsWithAnimation(
                    mSubItemHeight * (mSubItemCount + 1));
            if (mSubItemCount == 0) {
    ...
    /**
     * Remove the given view representing the custom_sub_item custom_item,
     * with animation. Should be an existing custom_sub_item custom_item.
     *
     * @param view The custom_sub_item custom_item to be removed.
     */
    ...
    public void setIndicatorColor(int color) {
        ((GradientDrawable) findViewById(
                R.id.icon_indicator_top).getBackground().mutate()).setColor(color);
        ((GradientDrawable) findViewById(
                R.id.icon_indicator_bottom).getBackground().mutate()).setColor(color);
        findViewById(
                R.id.icon_indicator_middle).setBackgroundColor(color);
    }
    ...
    /**
     * Creates a custom_sub_item custom_item based on sub_item_layout Layout,
     * set as ExpandingItem layout attribute.
     *
     ...
     * set as ExpandingItem layout attribute.
     *
     * @param position The position to add the new Item. Position should not be
     *                 greater than the list size.
     *                 If position is -1, the custom_item will be added in the end
     *                 of the list.
     * @return The inflated custom_sub_item custom_item view.
     */
    ...
    public View createSubItem(int position) {
        ...
        if (position > mBaseSubListLayout.getChildCount()) {
            throw new IllegalArgumentException("Cannot add an custom_item at position "
                    + position
                    + ". List size is " + mBaseSubListLayout.getChildCount());
        }

        ViewGroup subItemLayout = (ViewGroup) mInflater.inflate(
                mSubItemLayoutId, mBaseSubListLayout, false);
        if (position == -1) {
     ...
     *
     * @param startingPos The position from where the animation should start.
     *                    Useful when removing custom_sub_item items.
     */
    private void expandIconIndicator(float startingPos) {
        if (mIndicatorBackground != null) {
            final int totalHeight =
                    (mSubItemHeight * mSubItemCount) -
                            mIndicatorHeight / 2 + mItemHeight / 2;
     ...
     *
     * @param subItem  The view representing the custom_sub_item custom_item to
     *                 be removed.
    ...
    private void animateSubItemAppearance(final View subItem, boolean isAdding) {
        ...
        alphaAnimation.setDuration(isAdding ?
                mAnimationDuration * 2 : mAnimationDuration / 2);
     ...
     * @param index     The custom_sub_item custom_item index.
     *                  Needed to calculate delays.
     */
    ...
    private void animateViewAlpha(final ViewGroup viewGroup, int index) {
        ...
        ValueAnimator animation = mSubItemsShown ?
                ValueAnimator.ofFloat(0f, 1f) :
                ValueAnimator.ofFloat(1f, 0f);
        animation.setDuration(mSubItemsShown ?
    ...
    /**
     * Method to adjust Item position in parent if its custom_sub_item items
     * are outside screen.
     */
    ...
    private void inflateLayouts(Context context) {
        ...
        if (mItemLayoutId != 0) {
            mItemLayout = (ViewGroup) mInflater.inflate(
                    mItemLayoutId, mBaseLayout, false);
        }
    ...
    private void setIndicatorBackgroundSize() {
        CustomViewUtils.setViewHeight(
                mBaseLayout.findViewById(R.id.icon_indicator_top), mIndicatorHeight);
        CustomViewUtils.setViewHeight(
                mBaseLayout.findViewById(R.id.icon_indicator_bottom), mIndicatorHeight);
        CustomViewUtils.setViewHeight(
                mBaseLayout.findViewById(R.id.icon_indicator_middle), 0);


        CustomViewUtils.setViewWidth(
                mBaseLayout.findViewById(R.id.icon_indicator_top), mIndicatorWidth);
        CustomViewUtils.setViewWidth(
                mBaseLayout.findViewById(R.id.icon_indicator_bottom), mIndicatorWidth);
        CustomViewUtils.setViewWidth(
                mBaseLayout.findViewById(R.id.icon_indicator_middle), mIndicatorWidth);

        mItemLayout.post(new Runnable() {
            @Override
            public void run() {
                CustomViewUtils.setViewMargin(
                        mIndicatorContainer,
        ...
        CustomViewUtils.setViewMarginTop(
                mBaseLayout.findViewById(
                        R.id.icon_indicator_middle), (-1 * mIndicatorHeight / 2));
        CustomViewUtils.setViewMarginTop(
                mBaseLayout.findViewById(
                        R.id.icon_indicator_bottom), (-1 * mIndicatorHeight / 2));
    ...
    private void readAttributes(Context context, AttributeSet attributeSet) {
        ...
        try {
            mItemLayoutId = array.getResourceId(
                    R.styleable.CustomItem_item_layout, 0);
            mSeparatorLayoutId = array.getResourceId(
                    R.styleable.CustomItem_separator_layout, 0);
            mSubItemLayoutId = array.getResourceId(
                    R.styleable.CustomItem_sub_item_layout, 0);
            mIndicatorHeight = array.getDimensionPixelSize(
                    R.styleable.CustomItem_indicator_height, 0);
            mIndicatorWidth = array.getDimensionPixelSize(
                    R.styleable.CustomItem_indicator_width, 0);
            mIndicatorMarginLeft = array.getDimensionPixelSize(
                    R.styleable.CustomItem_indicator_margin_left, 0);
            mIndicatorMarginRight = array.getDimensionPixelSize(
                    R.styleable.CustomItem_indicator_margin_right, 0);
            mIndicatorMarginTop = array.getDimensionPixelSize(
                    R.styleable.CustomItem_indicator_margin_top, 0);
            mIndicatorMarginBottom = array.getDimensionPixelSize(
                    R.styleable.CustomItem_indicator_margin_bottom, 0);
            mShowIndicator = array.getBoolean(
                    R.styleable.CustomItem_show_indicator, true);
            mShowAnimation = array.getBoolean(
                    R.styleable.CustomItem_show_animation, true);
            mStartCollapsed = array.getBoolean(
                    R.styleable.CustomItem_start_collapsed, true);
            mAnimationDuration = array.getInt(
                    R.styleable.CustomItem_animation_duration, DEFAULT_ANIM_DURATION);
        } finally {
            array.recycle();
        }
    }
